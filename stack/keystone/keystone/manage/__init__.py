...
OBJECTS = ['user', 'tenant', 'role', 'service',
    'endpointTemplates', 'token', 'endpoint', 'credentials','billunit']
...
def process(*args):
    ...
    elif (object_type, action) == ('billunit', 'add'):
        if api.add_billunit(vcpu=args[1], ram=args[1], vdisk=args[1], date=args[1]):
            print "SUCCESS: billunit %s created." % object_id

    elif (object_type, action) == ('billunit', 'revoke'):
       print_table(('vcpu', 'ram', 'vdisk', 'date'), api.revoke_billunit())
