...
class IdentityService(object):
    ...
    def get_bill_unit(self, billunit_date):
        dbillunit = api.BILLER.get(billunit_date)
        if not dbillunit:
            raise fault.ItemNotFoundFault("The Bill could not be found")
	LOG.info('keystone logic servie py get_bill_unit dbillunit id:%d vcpu:%d ram:%d vdisk:%d changed_on %s enabled:%d'% (dbillunit.id, dbillunit.vcpu, dbillunit.ram, dbillunit.vdisk, dbillunit.changed_on, dbillunit.enabled))
        return Bill_Unit(id=dbillunit.id, vcpu=dbillunit.vcpu, ram=dbillunit.ram,
                     vdisk=dbillunit.vdisk, date=dbillunit.date, changed_on=dbillunit.changed_on, enabled=dbillunit.enabled)
                     
    
    def create_bill_unit(self, admin_token, billunit):
        self.__validate_admin_token(admin_token)
        ...
        dbillunit = models.BillUnit()
        dbillunit.vcpu = billunit.vcpu
        dbillunit.ram = billunit.ram
        dbillunit.vdisk = billunit.vdisk
        dbillunit.date = billunit.date
        dbillunit.changed_on = billunit.changed_on
        dbillunit.enabled = billunit.enabled
        dbillunit= api.BILLER.create(dbillunit)
        billunit.id = dbillunit.id
        return billunit

    def get_instance_bill(self, instance_id):
        dinstacne = api.BILLER.get_instance_bill(instance_id)
        if not dinstacne:
            raise fault.ItemNotFoundFault("The Instance Bill could not be found")
        LOG.info('keystone logic servie py get_instance_bill dinstance id:%d name :%s vcpu:%d ram:%d vdisk:%d changed_on %s enabled:%d'% (dinstacne.id, dinstacne.name, dinstacne.total_vcpu, dinstacne.total_ram, dinstacne.total_vdisk, dinstacne.changed_on, dinstacne.enabled))
        return Instance_Bill(id=dinstacne.id, name=dinstacne.name, total_vcpu=dinstacne.total_vcpu, total_ram=dinstacne.total_ram,
                     total_vdisk=dinstacne.total_vdisk, changed_on=dinstacne.changed_on, enabled=dinstacne.enabled)


    def create_instance_bill(self, admin_token, instance):
        self.__validate_admin_token(admin_token)
        #self.validate_and_fetch_user_tenant(user.tenant_id)
        if not isinstance(instance, Instance_Bill):
           raise fault.BadRequestFault("Expecting a Instance_Bill")
        dinstacne = models.InstanceBills()
        dinstacne.id = instance.id
        dinstacne.total_vcpu = instance.total_vcpu
        dinstacne.total_ram = instance.total_ram
        dinstacne.total_vdisk = instance.total_vdisk
        dinstacne.total_cost = instance.total_cost
        dinstacne.name = instance.name
        dinstacne.changed_on = instance.changed_on
        dinstacne.enabled = instance.enabled
        dinstacne= api.BILLER.create_instance_bill(dinstacne)
       # instance.id = dinstacne.id
        return instance

    def get_user_bill(self, user_id, month):
        duser = api.BILLER.get_user_bill(user_id, month)
        if not dinstacne:
            raise fault.ItemNotFoundFault("The User Bill could not be found")
        LOG.info('keystone logic servie py get_user_bill dinstance id:%d name :%s vcpu:%d ram:%d vdisk:%d changed_on %s enabled:%d'% (duser.id, dduser.user_id, duser.total_vcpu, duser.total_ram, duser.total_vdisk,  duser.enabled))
        return User_Bill(id=duser.id, user_id=duser.user_id, tenant_id=duser.tenant_id, total_cpu=duser.total_cpu, total_ram=duser.total_ram, total_vdisk=duser.total_vdisk, total_usage=duser.total_cost, bill_month=duser.bill_month, enabled=duser.enabled)


    def create_user_bill(self, admin_token, user_bill):
        self.__validate_admin_token(admin_token)
        self.validate_and_fetch_user_tenant(user_bill.tenant_id)
        if not isinstance(user_bill, User_Bill):
           raise fault.BadRequestFault("Expecting a User_Bill")
        duser = models.UserBills()
        duser.total_vcpu = user_bill.total_vcpu
        duser.total_ram = user_bill.total_ram
        duser.total_vdisk = user_bill.total_vdisk
        duser.total_usage = user_bill.total_cost
        duser.user_id = user_bill.user_id
        duser.tenant_id = user_bill.tenant_id
        duser.bill_month = user_bill.bill_month
        duser.enabled = user_bill.enabled
        duser= api.BILLER.create_user_bill(duser)
        user_bill.id = duser.id
        return user_bill
