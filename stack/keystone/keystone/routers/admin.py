...
from keystone.controllers.biller import BillController
...
class AdminApi(wsgi.Router):
    ...
    def __init__(self, options):
        ...
        #Bill operations
        bill_controller = BillController(options)
        mapper.connect("/billers/{biller_date}",
                    controller=bill_controller,
                    action="get_billunit",
                    conditions=dict(method=["GET"]))
        mapper.connect("/billers/create",
                    controller=bill_controller,
                    action="create_billunit",
                    conditions=dict(method=["POST"]))
        mapper.connect("/billers/{instance_id}/instance",
                    controller=bill_controller,
                    action="get_instance_bill",
                    conditions=dict(method=["GET"]))
        mapper.connect("/billers",
                    controller=bill_controller,
                    action="create_instance_bill",
                    conditions=dict(method=["POST"]))
        mapper.connect("/billers/{user_id}/user_bill/{bill_month}",
                    controller=bill_controller,
                    action="get_user_bill",
                    conditions=dict(method=["GET"]))
        mapper.connect("/billers/user_bill",
                    controller=bill_controller,
                    action="create_user_bill",
                    conditions=dict(method=["POST"]))
