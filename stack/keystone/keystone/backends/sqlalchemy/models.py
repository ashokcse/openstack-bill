...
from sqlalchemy import Column, String, Integer, ForeignKey, \
    UniqueConstraint, Boolean, DateTime, Float
...
class BillUnit(Base, KeystoneBase):
    __tablename__ = 'billunit'
    __api__ = 'biller'
    id = Column(Integer, primary_key=True, autoincrement=True)
...
class InstanceBills(Base, KeystoneBase):
    __tablename__ = 'instance_bills'
    __api__ = 'biller'
    id = Column(Integer, primary_key=True, autoincrement=True)
    #start_date  = Column(DateTime)
    name = Column(String(40))
    #user_id = Column(String(254))
    total_vcpu = Column(Float)
    total_ram = Column(Float)
    total_vdisk = Column(Float)
    changed_on = Column(DateTime)
    total_cost = Column(Float)
    enabled = Column(Boolean)

class UserBills(Base, KeystoneBase):
    __tablename__ = 'user_bills'
    __api__ = 'biller'
    id = Column(Integer, primary_key=True, autoincrement=True)
    user_id = Column(Integer)# ForeignKey('users.id')) #start_date  = Column(DateTime)
    tenant_id = Column(Integer, ForeignKey('tenants.id'), primary_key=True)   
    total_vcpu = Column(Float)
    total_ram = Column(Float)
    total_vdisk = Column(Float)
    bill_month = Column(DateTime, primary_key=True)
    total_usage = Column(Float)
    status = Column(Boolean)
    enabled = Column(Boolean)

#    class Meta:
#        unique_together = (('bill_month', 'tenant_id'),)
