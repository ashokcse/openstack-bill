...
class Biller(APIResourceWrapper):
    """Simple wrapper around openstackx.auth.tokens.Token"""
    _attrs = ['id', 'vcpu', 'ram', 'vdisk','date','changed_on','enabled']

class InstanceBiller(APIResourceWrapper):
    """Simple wrapper around openstackx.auth.tokens.Token"""
    _attrs = ['id', 'name', 'total_vcpu', 'total_ram', 'total_vdisk', 'total_cost', 'changed_on', 'enabled']

class TenantBiller(APIResourceWrapper):
    """Simple wrapper around openstackx.auth.tokens.Token"""
    _attrs = ['id', 'tenant_id', 'total_vcpu', 'total_ram', 'total_vdisk', 'total_cost', 'bill_month', 'enabled']
...
def biller_create(request, vcpu, ram, vdisk, date, created_on, enabled):
    return Biller(account_api(request).billers.create(
            vcpu, ram, vdisk, date, created_on, enabled))

def biller_get(request, biller_date):
    return Biller(account_api(request).billers.get(biller_date))

def biller_create_ibill(request, id, name, total_vcpu, total_ram, total_vdisk, total_cost, created_on, enabled):
    return InstanceBiller(account_api(request).billers.create_ibill(
            id, name, total_vcpu, total_ram, total_vdisk, total_cost, created_on, enabled))

def biller_get_ibill(request, instance_id):
    return InstanceBiller(account_api(request).billers.get_ibill(instance_id))

def biller_create_ubill(request, id, tenant_id, total_vcpu, total_ram, total_vdisk, total_cost, bill_month, enabled):
    return TenantBiller(account_api(request).billers.create_ubill(
            id, None, tenant_id, total_vcpu, total_ram, total_vdisk, total_cost, bill_month, enabled))

def biller_get_ubill(request, instance_id):
    return TenantBiller(account_api(request).billers.get_ubill(tenent_id, bill_month))
