...
@login_required
def index(request, tenant_id=None):
    today = utils.today()
    ...
    show_terminated = request.GET.get('show_terminated', True)
    ...
    if unit_flag == 0:
        messages.info(request, 'Current cost will be  available soon')
    else :
      LOG.info(' ---$$---user usage unit cost -$$$$---- %s -----$-------- ' %(unit_cost.__dict__))
    ...
    if request.GET.get('format', 'html') == 'csv':
        template_name = 'django_openstack/dash/bills/usage.csv'
        mimetype = "text/csv"
    else:
        template_name = 'django_openstack/dash/bills/index.html'
        mimetype = "text/html"
